...
function logTest(endpoint: string, method: string, status: number, success: boolean, message: string) {
  const result: TestResult = { endpoint, method, status, success, message };
  results.push(result);
  const emoji = success ? '✅' : '❌';
