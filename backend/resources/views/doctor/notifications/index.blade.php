...
@section('title', 'Clinical Notifications')
@section('page_title', 'Clinical Alerts & Notifications')

@section('content')
<div class="space-y-6 max-w-4xl">
    <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs flex items-center justify-between">
        <div>
            <h2 class="text-base font-semibold text-slate-900">Clinical Notifications & Alerts</h2>
            <p class="text-xs text-slate-500">Appointment updates, teleconsultation requests, and critical clinical notices</p>
        </div>

        @if ($unreadCount > 0)
            <form method="POST" action="{{ route('doctor.notifications.read-all') }}">
                @csrf
                <button type="submit" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-xl text-xs transition-colors">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="flex items-center gap-2 mt-2">
                        <span class="px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wider bg-slate-100 text-slate-600">
                            {{ str_replace('_', ' ', $notif->notification_type ?? 'clinical') }}
                        </span>
